...
class Gesamtbilanz {
  ...
  aktualisieren(eintraege, fixed_expenses) {
    // Add fixed_expenses as an argument
    this._einnahmen = 0;
    this._ausgaben = 0;
    this._bilanz = 0;
    ...
    // Use fixed_expenses instead of fixedExpenses
    fixed_expenses.forEach((expense) => {
      switch (expense.category) {
        case "income":
          this._einnahmen = this._einnahmen + expense.amount;
          this._bilanz = this._bilanz + expense.amount;
          break;
        case "spending":
          this._ausgaben = this._ausgaben + expense.amount;
          this._bilanz = this._bilanz - expense.amount;
          break;
        default:
          console.log(`The category "${expense.category}" is not known.`);
          break;
      }
    });
  ...
  _html_generieren() {
    ...
    einnahmen_betrag.textContent = `${(this._einnahmen / 100)
      .toFixed(2)
      .replace(/\./, ",")} €`;
    ...
    ausgaben_betrag.textContent = `${(this._ausgaben / 100)
      .toFixed(2)
      .replace(/\./, ",")} €`;
    ...
    bilanz_betrag.textContent = `${(this._bilanz / 100)
      .toFixed(2)
      .replace(/\./, ",")} €`;
