...
class Monatslistensammlung {
    ...
    getEntriesByMonthAndYear(month, year) {
        let monatsliste = this._monatslisten.find((ml)=>ml.monat() == month && ml.jahr() == year);
        return monatsliste ? monatsliste.getEintraege() : [];
    }
    _eintrag_hinzufuegen(eintrag) {
        ...
        if (!monatsliste_vorhanden) this._monatsliste_hinzufuegen(eintragsjahr, eintragsmonat, eintrag);
    }
    ...
    _monatslisten_sortieren() {
        this._monatslisten.sort((monatsliste_a, monatsliste_b)=>{
            if (monatsliste_a.jahr() < monatsliste_b.jahr()) return 1;
            else if (monatsliste_a.jahr() > monatsliste_b.jahr()) return -1;
            else {
                if (monatsliste_a.monat() < monatsliste_b.monat()) return 1;
                else return -1;
            }
        });
    }
    ...
    _entfernen() {
        ...
        if (monatslistensammlung !== null) monatslistensammlung.remove();
    }
    anzeigen() {
...
//# sourceMappingURL=index.7d426e73.js.map
