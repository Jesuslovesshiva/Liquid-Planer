...
class Haushaltsbuch {
    ...
    showFixkostenPopup() {
        ...
        const monatsliste = this._monatslistensammlung.getEntriesByMonthAndYear(currentMonth, currentYear);
        const fixkostenPopup = new FixkostenPopup(monatsliste);
        fixkostenPopup.show();
    }
    _ShowFixKosten() {
        const showFixkostenPopupButton = document.getElementById("show-fixkosten-popup");
        if (showFixkostenPopupButton) showFixkostenPopupButton.addEventListener("click", (e)=>{
            e.preventDefault();
    ...
    eintrag_entfernen(timestamp) {
        let start_index;
        for(let i = 0; i < this._eintraege.length; i++)if (this._eintraege[i].timestamp() === parseInt(timestamp)) {
            start_index = i;
            break;
        }
    ...
    addFixedExpense(expenseData) {
        const newFixedExpense = new FixedExpense(expenseData.title, expenseData.amount, expenseData.category);
        this._fixedExpenses.push(newFixedExpense);
        this._speichern();
    }
    _wiederherstellen() {
        let gespeicherte_eintraege = localStorage.getItem("eintraege");
        if (gespeicherte_eintraege !== null) JSON.parse(gespeicherte_eintraege).forEach((eintrag)=>{
        ...
        if (storedFixedExpenses !== null) JSON.parse(storedFixedExpenses).forEach((expense)=>{
...
//# sourceMappingURL=index.2ed6c1a4.js.map
