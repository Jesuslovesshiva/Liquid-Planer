...
class Monatsliste {
    ...
    _eintraege_sortieren() {
        this._eintraege.sort((eintrag_a, eintrag_b)=>{
            if (eintrag_a.datum() > eintrag_b.datum()) return -1;
            else if (eintrag_a.datum() < eintrag_b.datum()) return 1;
            else {
                if (eintrag_a.timestamp() > eintrag_b.timestamp()) return -1;
                else return 1;
            }
        });
    }
    _bilanzieren() {
        ...
        this._eintraege.forEach((eintrag)=>{
            if (eintrag.typ() === "einnahme") monatsbilanz += eintrag.betrag();
            else monatsbilanz -= eintrag.betrag();
        });
    ...
    _html_generieren() {
        ...
        if (this._bilanz >= 0) monatsbilanz.setAttribute("class", "monatsbilanz positiv");
        else monatsbilanz.setAttribute("class", "monatsbilanz negativ");
...
//# sourceMappingURL=index.f8902a46.js.map
